...
# 核心代码
# import example
import Xlsx_n from 'songyz-xlsx'
...
export const importSlsx = (file, opts) => {
    return new Promise(function (resolve, reject) {
        ...
        reader.onload = function (e) {
            ...
            const wb = Xlsx_n.read(e.target.result, opts);
            resolve(Object.keys(wb.Sheets).map(key => Xlsx_n.utils.sheet_to_json(wb.Sheets[key])).reduce((prev, next) => prev.concat(next)))
        }
...
export const exportXlsx = (dataArray, fileName) => {
    ...
    var buffer = Xlsx_n.write(wb, { bookType: type, bookSST: false, type: 'buffer' });
